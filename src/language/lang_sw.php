<?hh // strict

/* HH_IGNORE_ERROR[1002] */
$translations = array(
  'date and time format' =>
    'Saa:DK:Sek Siku Mz/Sk/Mw', //used by date() function
  //Transations for IndexController
  'Facebook CTF' =>
    'Facebook CTF',
  'Conquer the world' =>
    'miliki ulimwengu',
  'Play' =>
    'Cheza',
  'Welcome to the Facebook Capture the Flag Competition. By clicking "Play," you will be entered into the official CTF challenge. Good luck in your conquest.' =>
    'Karibu katika Mfumo wa kimashindano wa Facebook. Kwa kubofya "Cheza," Utaingizwa katika shindano rasmi la CTF. Nakutakia kila la kheri',
  'Get ready for the CTF to start and access the gameboard now!' =>
    'Kuwa tayari kwa CTF ili kuanza na kufikia ubao wa mchezo sasa!',
  'Gameboard' =>
    'Ukurasa wa Mchezo',
  'Register Team' =>
    'Sajili Timu',
  'Get ready for the CTF to start and register your team now!' =>
    'Kaatayari na jiandae kwa CTF kuanza na Sajili Timu yako sasa!',
  'Login' =>
    'Ingia',
  'Soon' =>
    'Hivi karibuni',
  'Upcoming Game' =>
    'Mchezo Unaokuja',
  '_days' =>
    '_siku',
  '_hours' =>
    '_saa',
  '_minutes' =>
    '_dakika',
  '_seconds' =>
    '_Sekunde',
  'Official CTF Rules' =>
    'Kanuni Rasmi za CTF',
  'Following actions are prohibited, unless explicitly told otherwise by event Admins.' =>
    'Yafuatayo ni vitendo visivyoruhusiwa, isipokuwa pale tu umeambiwa vinginevyo na Msimamizi wa tukio.',
  'Rule' =>
    'Kanuni',
  'Cooperation' =>
    'Ushirikiano',
  'No cooperation between teams with independent accounts. Sharing of keys or providing revealing hints to other teams is cheating, don’t do it.' =>
    'Hakuna ushirikiano kati ya timu zilizo na akaunti tofauti. Kushiriki funguo au kujaribu kutoa vidokezo kwa timu nyingine ni udanganyifu, usifanye hivyo',
  'Attacking Scoreboard' =>
    'Ukurasa wa kufanyia mashambulizi',
  'No attacking the competition infrastructure. If bugs or vulns are found, please alert the competition organizers immediately.' =>
    'Hairihusiwi kuvamia miundo mbinu ya shindano. Kama kuna udhaifu au kasoro imeonekana, tafadhali mjulishe kiongozi au muongoza shindano mara moja.',
  'Sabotage' =>
    'Hujuma',
  'Absolutely no sabotaging of other competing teams, or in any way hindering their independent progress.' =>
    'Hairuhusiwi kabisa kuhujumu timu nyingine zilizo katika shindano, au kwa namna yoyote ili kujaribu kuzuia maendeleo yao binafsi katika shindano hili.',
  'Bruteforcing' =>
    'Kuingilia kwa nguvu',
  'No brute forcing of challenge flag/ keys against the scoring site.' =>
    'Hairuhusiwi kuingilia kwa nguvu flag/ keys kinyume na inavyo takiwa kwenye ukurasa wa alama.',
  'Denial Of Service' =>
    'Kukataa Huduma',
  'DoSing the CTF platform or any of the challenges is forbidden.' =>
    'Kuzuia huduma za mfumo wa CTF au shindano lolote lile imezuiliwa kabisa.',
  'Legal' =>
    'Kisheria',
  'Disclaimer' =>
    'Kanusho',
  'By participating in the contest, you agree to release Facebook and its employees, and the hosting organization from any and all liability, claims or actions of any kind whatsoever for injuries, damages or losses to persons and property which may be sustained in connection with the contest. You acknowledge and agree that Facebook et al is not responsible for technical, hardware or software failures, or other errors or problems which may occur in connection with the contest.' =>
    'Kwa kushiriki katika shindano hili, unakubali kuiachia huru Facebook na wafanyakazi wake, pamoja na taasisi inayo andaa shindano kutoka katika dhima, ya madai au matendo ya aina yoyote ile la kimajeraha, uharibifu, au hasara ya watu na mali, ambayo wanaweza kuhalalishwa kwa kuwa na muunganiko na shindano hili. Unatambua na kukubali kuwa Facebook, Taasisi inayoandaa shindano,pamoja na wenzake haihusiki na makosa ya kiufundi, ya kivifaa au ya kiprogramu, au makosa yoyote au matatizo ambayo yatotokea na kuwa na muingiliano wa  aina yoyote na shindano hili.',
  'If you have any questions about what is or is not allowed, please ask an organizer.' =>
    'Kama una maswali yoyote kuhusu kinacho ruhusiwa na kisicho ruhusiwa , tafadhali muulize muandaaji.',
  'Have fun!' =>
    'Kuwa na furaha!',
  'Name' =>
    'Jina',
  'Email' =>
    'Barua pepe',
  'Token' =>
    'Tuzo',
  'Team Registration' =>
    'Usajili wa Timu',
  'Team Name' =>
    'Jina la Timu',
  'Password' =>
    'Nenosiri',
  'Choose an Emblem' =>
    'Chagua Nembo',
  'Sign Up' =>
    'Jiunge',
  'Register to play Capture The Flag here. Once you have registered, you will be logged in.' =>
    'Jisajili kucheza Capture The Flag hapa. Mara utakapo jisajili, utaingia',
  'Not Available' =>
    'Haipo',
  'Team Registration will be open soon, stay tuned!' =>
    'Usajili wa Timu utafunguliwa hivi karibuni, makinika!',
  'Try Again' =>
    'Jaribu Tena',
  'Select' =>
    'Chagua',
  'Team Login' =>
    'Kuingia kwa Timu',
  'Please login here. If you have not registered, you may do so by clicking "Sign Up" below. ' =>
    'Tafadhali ingia hapa. Kama hujajisajili, unaweza kufanya hivyo kwa kubofya "Jiunge" hapa chini.',
  'Team Login will be open soon, stay tuned!' =>
    'Timu ya kuingia itakuwa wazi hivi karibuni, makinika!',
  'ERROR' =>
    'KOSA',
  'Start Over' =>
    'Anza upya',
  'Window is too small' =>
    'Dirisha ni dogo mno',
  'For the best CTF experience, please make window size bigger.' =>
    'Ili kufurahia vizuri CTF, tafadhali ongeza ukubwa wa dirisha.',
  'Thank you.' =>
    'Asante.',
  'Logout' =>
    'Ondoka',
  'Registration' =>
    'Usajili',
  'Play CTF' =>
    'Cheza CTF',
  'Rules' =>
    'Kanuni',
  //Translations for GameboardController
  'Admin' =>
    'msimamizi',
  'ADMIN' =>
    'MSIMAMIZI',
  'Navigation' =>
    'Uelekezi',
  'View Mode' =>
    'Hali ya Mwonekano',
  'View mode' =>
    'Hali ya mwonekano',
  'Tutorial' =>
    'Mafunzo',
  'Scoreboard' =>
    'ubao (Ukurasa)',
  'You' =>
    'Wewe',
  'Others' =>
    'Wengine',
  'All' =>
    'Wote',
  'Leaderboard' =>
    'Ukurasa wa Kiongozi',
  'Announcements' =>
    'Matangazo',
  'Teams' =>
    'Timu',
  'Filter' =>
    'Kichujio',
  'Activity' =>
    'Shughuli',
  'Game Clock' =>
    'Saa ya Mchezo',
  //Translations for AdminController
  'Auto' =>
    'Otomatiki',
  'All Categories' =>
    'Makundi yote',
  'Open' =>
    'Fungua',
  'Tokenized' =>
    'Isharisha',
  'Hour' =>
    'Saa',
  'Hours' =>
    'Saa',
  'Used by' =>
    'Imetumiwa na',
  'Used By' =>
    'Imetumiwa na',
  'Available' =>
    'Inapatikana',
  'Registration Tokens' =>
    'Usajili wa Tokens',
  'Create More' =>
    'Tengeneza Zaidi',
  'Export Available' =>
    'Uhamishaji Unapatikana',
  'Not started yet' =>
    'Bado haijaanza',
  'Configuration' =>
    'Usanidi',
  'Tokens' =>
    'Tokens',
  'Game Configuration' =>
    'Usanidi wa Mchezo',
  'OK' =>
    'Sawa',
  'status_' =>
    'hali_',
  'On' =>
    'Inawaka',
  'Off' =>
    'Imezima',
  'Player Names' =>
    'Majina ya Mchezaji',
  'Players Per Team' =>
    'Wanamichezo kwa timu',
  'Registration Type' =>
    'Aina ya Usajili',
  'Strong Passwords' =>
    'Manenosiri Thabiti',
  'Team Selection' =>
    'Uteuzi wa Timu',
  'Game' =>
    'Mchezo',
  'Scoring' =>
    'Uwekaji alama',
  'Progressive Cycle (s)' =>
    'Mzunguko(mizunguko) ya maendeleo',
  'Refresh Gameboard' =>
    'Onyesha upya Ubao wa mchezo',
  'Default Bonus' =>
    'Zawadi za ziada',
  'Bases Cycle (s)' =>
    'Mzunguko (Mizunguko) ya msingi',
  'Default Bonus Dec' =>
    'Point za ziada za kawaida Dec',
  'Timer' =>
    'Kitunza muda',
  'Server Time' =>
    'Muda wa server',
  'Game Duration' =>
    'Muda wa Mchezo',
  'Begin Time' =>
    'Muda wa kuanza',
  'Expected End Time' =>
    'Muda tegemewa wa kumaliza',
  'Language' =>
    'Lugha',
  'DELETE' =>
    'FUTA',
  'Delete' =>
    'FUTA',
  'No Announcements' =>
    'Hakuna Matangazo',
  'Game Controls' =>
    'Vidhibiti vya mchezo',
  'Write New Announcement here' =>
    'Andika matangazo mapya hapa',
  'Create' =>
    'tengeneza',
  'General' =>
    'Kwa jumla',
  'Back Up Database' =>
    'Hifadhi datamsingi (Database)',
  'Export Full Game' =>
    'Peleka nje mchezo mzima',
  'Import Full Game' =>
    'Ingiza Ndani Mchezo mzima',
  'Import Teams' =>
    'Ingiza timu',
  'Export Teams' =>
    'Toa timu',
  'Import Logos' =>
    'Ingiza Nembo',
  'Export Logos' =>
    'Toa Nembo',
  'Import Levels' =>
    'Ingiza Hatua',
  'Export Levels' =>
    'Toa Hatua',
  'Import Categories' =>
    'Ingiza Makundi',
  'Export Categories' =>
    'Toa makundi',
  'Levels' =>
    'Hatua',
  'New Quiz Level' =>
    'Hatua mpya za maswali',
  'Title' =>
    'Kichwa',
  'Question' =>
    'Swali',
  'Level title' =>
    'Kichwa cha Hatua',
  'Quiz question' =>
    'Kazi ya swali',
  'Country' =>
    'Nchi',
  'Answer' =>
    'Jibu',
  'Points' =>
    'Alama',
  'Hint' =>
    'Kidokezo',
  'Hint Penalty' =>
    'Adhabu ya Kidokezo',
  'EDIT' =>
    'REKEBISHA',
  'All Quiz Levels' =>
    'Hatua zote za kazi za maswali',
  'Filter By:' =>
    'Imechujwa na :',
  'All Status' =>
    'Matokeo yote',
  'Enabled' =>
    'Imewezeshwa',
  'Disabled' =>
    'Haijawezeshwa',
  'Quiz Level' =>
    'Hatua za maswali',
  'Show Answer' =>
    'Onesha Jibu',
  'Bonus' =>
    'ALama za ziada',
  '-Dec' =>
    '-Dec',
  'Save' =>
    'Hifadhi',
  'Quiz Management' =>
    'Usimamizi wa maswali',
  'Add Quiz Level' =>
    'Ongeza hatua (Ngazi) za maswali',
  'New Flag Level' =>
    'Ngazi mpya ya kibendera',
  'Description' =>
    'Maelezo mafupi',
  'Level description' =>
    'Ngazi (Hatua) ya maelezo mafupi',
  'Category' =>
    'Kundi',
  'Flag' =>
    'Kibendera',
  'flag' =>
    'Kibendera',
  'All Flag Levels' =>
    'Hatua zote za vibendera',
  'New Attachment:' =>
    'Viambatanishi vipya:',
  'Attachment' =>
    'Kiambatanishi',
  'Link' =>
    'Kiungo',
  'New Link:' =>
    'Kiungo kipya:',
  'Flag Level' =>
    'Hatua (Ngazi) ya kibendera',
  'Categories' =>
    'Makundi',
  '+ Attachment' =>
    '+ Kiambatanishi',
  '+ Link' =>
    '+ Kiungo',
  'Flags Management' =>
    'Usimamizi wa vibendera',
  'Add Flag Level' =>
    'Ongeza ngazi ya vibendera',
  'New Base Level' =>
    'Ngazi mpya ya msingi',
  'Keep Points' =>
    'Hifadhi alama',
  'Capture points' =>
    'Pata alama',
  'All Base Levels' =>
    'Hatua zote mpya za msingi',
  'Base Level' =>
    'Hatua msingi',
  'Bases Management' =>
    'Usimamizi wa msingi',
  'Add Base Level' =>
    'Ongeza hatua ya msingi',
  'New Category' =>
    'Kundi jipya',
  'Category: ' =>
    'Kundi: ',
  'Categories Management' =>
    'Usimamizi wa kundi',
  'Add Category' =>
    'Ongeza Kundi',
  'All Countries' =>
    'Nchi zote',
  'In Use' =>
    'Inatumika',
  'In use' =>
    'Inatumika',
  'Not Used' =>
    'Haitumiki',
  'Yes' =>
    'Ndio',
  'No' =>
    'Hapana',
  'ISO Code' =>
    'Utambulisho wa ISO',
  'Countries Management' =>
    'Usimamizi wa Nchi',
  'No Team Names' =>
    'Hakuna majina ya Timu',
  'time' =>
    'muda',
  'type' =>
    'aina',
  'pts' =>
    'pts',
  'Level' =>
    'Ngazi',
  'level' =>
    'ngazi',
  'No Scores' =>
    'Hakuna alama',
  'Attempt' =>
    'Jaribu',
  'No Failures' =>
    'Hakuna Kushindwaa',
  'Team' =>
    'Timu',
  'team' =>
    'timu',
  'Names' =>
    'Majina',
  'Scores' =>
    'Alama',
  'Failures' =>
    'Kushindwa',
  'New Team' =>
    'TImu mpya',
  'Team Logo' =>
    'Nembo ya timu',
  'Selected Logo:' =>
    'Chagua Nembo:',
  'Select Logo' =>
    'CHagua Nembo',
  'All Teams' =>
    'Timu zote',
  'Protected' =>
    'Imelindwa',
  'Score' =>
    'alama',
  'Change Password' =>
    'Badilisha nenosiri (Nywila)',
  'Admin Level' =>
    'Ngazi ya Kiongozi',
  'Visibility' =>
    'Uonekano',
  'Team Management' =>
    'Usimamizi wa timu',
  'Add Team' =>
    'Ongeza timu',
  'None' =>
    'Hakuna',
  'Logo Name' =>
    'Jina la Nembo',
  'Logo Management' =>
    'Usimamizi wa Nembo',
  'Session' =>
    'Nyakati',
  'Cookie' =>
    'VIpishi',
  'Creation Time' =>
    'Mda wa kutengeneza',
  'Last Access' =>
    'Mda wa mwisho wa kuingia',
  'Last Page Access' =>
    'Ukurasa wa mwisho kuingia',
  'Data' =>
    'Taarifa msingi',
  'Sessions' =>
    'Nyakati',
  'entry' =>
    'Ingia',
  'No Entries' =>
    'Hakuna kuingia',
  'Game Logs' =>
    'Nembo ya Mchezo',
  'Game Logs Timeline' =>
    'Mstari wa muda ya mchezo',
  'End Game' =>
    'Maliza mchezo',
  'Begin Game' =>
    'Anza mchezo',
  'Game Admin' =>
    'Kiongozi wa Mchezo',
  'Controls' =>
    'Vithibiti',
  'Quiz' =>
    'Zoezi',
  'Flags' =>
    'Vibendera',
  'Bases' =>
    'Msingi',
  'Countries' =>
    'Nchi',
  'Logos' =>
    'Nembo',
  //Translations for inc/* and inc/gameboard/*
  'captured' =>
    'iliyoshikwa',
  'Status' =>
    'Hali',
  'Completed' =>
    'Imeisha',
  'Remaining' =>
    'Imebakia',
  'Start' =>
    'Anza',
  'End' =>
    'Mwisho',
  'Rank' =>
    'Ngazi',
  'pts' =>
    'pts', //points
  'Your Rank' =>
    'Ngazi yako',
  'Your Score' =>
    'Alama Zako',
  'Everyone' =>
    'Kila mtu',
  'Your Team' =>
    'Timu yako',
  'Captured' =>
    'Imekamatwa',
  'Initiating' =>
    'Imeanzishwa',
  'run : > boot_sequence' =>
    'endesha : > hatua_zakuwaka',
  'Extracting' =>
    'Chimbua',
  //Translations for Utils.php's time_ago() function
  'just now' =>
    'Hivi sasa',
  'd' =>
    'Siku', //day
  'hr' =>
    'Saa', //hour
  'min' =>
    'Dak', //minute
  'sec' =>
    'sek', //second
  'ds' =>
    'Sk', //days
  'hrs' =>
    'Saa', //hours
  'mins' =>
    'Dak', //minutes
  'secs' =>
    'Sek', //seconds
  'ago' =>
    'iliyopita',
  //Translations for ModalControllers
  'begin_' =>
    'anza_',
  'Are you sure you want to kick off the game? Logs will be cleared and progressive scoreboard will start' =>
    'Una uhakika unataka kuanza mchezo? Kumbukumbu zitafutwa na taarifa za maendeleo katika ukurasa wa matokeo zitanza kuwekwa',
  'end_' =>
    'mwisho_',
  'Are you sure you want to finish the current game?' =>
    'Una uhakika unataka kumaliza mchezo wa sasa?',
  'Are you sure you want to logout from the game?' =>
    'Una uhakika unataka kutoka nje ya mchezo?',
  'Saved' =>
    'Imehifadhiwa',
  'All changes have been successfully saved.' =>
    'Marekebisho yote yamehifadhiwa.',
  'Error' =>
    'Kosa',
  'Sorry your form was not saved. Please correct the all errors and save again.' =>
    'Samahani fomu haijahifadhi.Tafadhali rekebisha makosa yote na jaribu tena',
  'cancel_' =>
    'Batilisha_',
  'Are you sure you want to cancel? You have unsaved changes that will be reverted.' =>
    'Una uhakika unataka kubatilisha? Una mabadiliko ambayo hayajahifadhiwa ambayo yataondolewa.',
  'choose_logo' =>
    'chagua_nembo',
  'captured_' =>
    'imepatwa_',
  'flag_owner_' =>
    'mmiliki_wabendera_',
  'INACTIVE' =>
    'HAIKOSAWA',
  'PTS' =>
    'PTS',
  'category' =>
    'kundi',
  'capture_' =>
    'pata_',
  'Insert your answer' =>
    'Weka jibu lako',
  'Request Hint' =>
    'Omba dokezo',
  'Submit' =>
    'Wasilisha',
  'hint_' =>
    'dokezo_',
  'first_capture' =>
    'pata_kwa mara ya kwanza',
  'completed_by' =>
    'imekamilishwa_na',
  'scoreboard_' =>
    'ukurasa wa alama_',
  'filter_' =>
    'chuja_',
  'rank_' =>
    'ngazi_',
  'team_name_' =>
    'JIna_latimu_',
  'quiz_pts_' =>
    'pts_zakazi_',
  'flag_pts_' =>
    'pts_zabendera_',
  'base_pts_' =>
    'pts_msingi_',
  'total_pts_' =>
    'pts_zajumla_',
  'team_' =>
    'timu_',
  'team_members' =>
    'wanachama_watimu',
  'base_pts' =>
    'pts_zamsingi',
  'quiz_pts' =>
    'pts_zakazi',
  'flag_pts' =>
    'pts_bendera',
  'total_pts' =>
    'pts_zajumla',
  'Tool bars are located on all edges of the gameboard. Tap a category to expand and close each tool bar.' =>
    'Zana zilizopo katika mche zipo katika kona zote za ukurasa wa mchezo. Bonyeza katika kundi ulitakalo kulitanua zaidi na ufunnge kila zana mche.',
  'Tool_Bars' =>
    'Zana_Mche',
  'Tap the "Game Clock" to keep track of time during gameplay. Don’t let time get the best of you.' =>
    'Bonyeza "Saa ya Mchezo" ili kuangalia muda wakati unapocheza. Usiache muda ukushinde.',
  'Game_Clock' =>
    'Saa_Mchezo',
  'Countries marked with an ' =>
    'Nchi zilizo wekewa alama ya ',
  'are captured by you.' =>
    'zilizo kamatwa na wewe.',
  ' are owned by others.' =>
    ' Zinamilikiwa na wengine.',
  'Captures' =>
    'zilizokamatwa',
  'Tap Plus[+] to Zoom In. Tap Minus[-] to Zoom Out.' =>
    'Bonyeza alama ya [+] kukuza zaidi. Bonyeza alama ya [-] kupunguza ukubwa.',
  'Click and Drag to move left, right, up and down.' =>
    'Bonyeza na vuta kwenda kushoto, kulia, juu na chini.',
  'Zoom' =>
    'Ongeza',
  'Tap Forward Slash [/] to activate computer commands. A list of commands can be found under "Rules".' =>
    'Bonyeza alama ya [/] ili kuanzisha amri za kompyuta. Oradha ya amri zinapatikana chini ya "Kanuni".',
  'Command_Line' =>
    'Mstari_wa Amri',
  'Click "Nav" to access main navigation links like Rules of Play, Registration, Blog, Jobs & more.' =>
    'Bonyeza "Nav" ili kufikia mwongozo mkuu wa viunganishi kama vile kanuni za mchezo, Usajili, blogu, Kazi na mengine Mengi.',
  'Track your competition by clicking "scoreboard" to access real-time game statistics and graphs.' =>
    'Fuatilia shindano lako kwa kubonyeza "Ubao wa alama" kuweza kuona takwimu halisi.',
  'Have fun, be the best and conquer the world.' =>
    'Kuwa na furaha, kuwa bora na uishinde dunia nzima.',
  'Game_On' =>
    'Mchezo_Umeanza',
  'tutorial_' =>
    'mafunzo_',
  'Next' =>
    'Inayofuata',
  'Skip to play' =>
    'Ruka mpaka kwenye kucheza',
  'Powered By Facebook' =>
    'Imewezeshwa na Facebook',
  'Active Directory / LDAP' =>
    'Active Directory / LDAP',
  'LDAP Server' =>
    'LDAP Server',
  'LDAP Port' =>
    'LDAP Port',
  'LDAP Domain' =>
    'LDAP Domain',
);
